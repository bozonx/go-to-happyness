...
static func _test_construction_site_uses_building_entrance() -> void:
	...
	var expected_positions := BuildingAccessPoints.construction_local_positions(blueprint)
	assert(not expected_positions.is_empty())
	var positions: Array = site.node.get_meta("service_positions")
	assert(positions.size() == expected_positions.size(), "Construction site should expose authored construction doors")
