...
func send_selected_resident_to_outside_work() -> void:
	if not is_instance_valid(game.selected_builder) or game.selected_builder.is_player_controlled:
		game.update_interface("Select an AI-controlled Courier before sending them to outside work.")
		return
	if not game.selected_builder.can_handle_entry_logistics() or not game.simulation_tick_controller.is_work_time():
		game.update_interface("Outside work requires a Courier.")
		return
	var worker_id := game.selected_builder.get_stable_id()
	if game.outside_workers.has(worker_id):
		game.update_interface("This resident is already working in a neighboring settlement.")
		return
	var reward := outside_work_reward()
	if game.courier_dispatcher.task_for(game.selected_builder) != null:
		game.update_interface("Courier is already assigned to a logistics task.")
		return
	...
	game.update_interface("Outside work assigned. The courier is heading to the entrance sign.")
...
func on_outside_work_departed(worker: Citizen) -> void:
	...
	game.update_interface("Courier left for outside work and will return in 24 hours with %d coins." % reward)
...
func return_outside_workers() -> void:
	...
	for worker_id in game.outside_workers.keys():
		...
		returned_any = true
		game.update_interface("A resident returned from outside work with %d coins." % reward)
