...
func _init() -> void:
	super(&"gathering", &"work.gathering.worker", "Gather resource for warehouse")


func score(snapshot: WorldSnapshot, citizen: CitizenSnapshot, order: CitizenOrder, blackboard: AIBlackboard) -> float:
	if citizen == null or order == null or order.kind != id:
		return 0.0
	if order.issuer == &"player":
		return clampf(order.priority, 0.0, 1.0)
	return super.score(snapshot, citizen, order, blackboard)
...
func build_task(_snapshot: WorldSnapshot, _citizen: CitizenSnapshot, order: CitizenOrder, _blackboard: AIBlackboard) -> BehaviorTask:
	...
	return BehaviorTask.new(id, SequenceStepScript.new([
		MoveToStepScript.new(move_target, 0.25, [&"gathering.source", source_id]),
