...
func collect(ctx: FacadeContext) -> Dictionary:
	...
	var farming_can_start: bool = farming_worker and actor_work_time and ctx.simulation.storage_routing_service.has_storage_room_for_role("farming") and farming_position != Vector3.INF and farming_warehouse_position != Vector3.INF and ctx.simulation.is_route_reachable(actor.global_position, farming_position) and ctx.simulation.is_route_reachable(farming_position, farming_warehouse_position)
