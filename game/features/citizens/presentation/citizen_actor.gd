...
static var AI_JUMP_VELOCITY: float = HumanoidMobility.JUMP_VELOCITY
...
func _ready() -> void:
	...
	var rng: RandomNumberGenerator = random if random != null else (simulation.get("random") as RandomNumberGenerator if simulation != null else null)
	if gender.is_empty():
...
func _process_to_workplace(delta: float) -> void:
	if _move_to(workplace_position, delta):
		if resource_type == ResourceIds.WOOD:
			var count := 1
			var forestry_rng: RandomNumberGenerator = simulation.get("random") as RandomNumberGenerator if simulation != null else null
...
func begin_role_recheck_cooldown() -> void:
	if has_no_permanent_work() and daily_order_role.is_empty():
		var recheck_rng: RandomNumberGenerator = simulation.get("random") as RandomNumberGenerator if simulation != null else null
...
func _process_gathering(delta: float) -> void:
	if _work(delta):
		...
		if resource_type == ResourceIds.LOGS:
			...
			var gather_rng: RandomNumberGenerator = simulation.get("random") as RandomNumberGenerator if simulation != null else null
