...
static func success(next_waypoints: Array[Vector3], next_arrival_position: Vector3, next_grid_revision := -1) -> RouteResult:
...
static func unreachable(next_grid_revision := -1) -> RouteResult:
	var result := RouteResult.new()
	result.grid_revision = next_grid_revision
	return result
