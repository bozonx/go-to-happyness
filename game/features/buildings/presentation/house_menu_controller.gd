...
func show_house_menu() -> void:
	...
	simulation.ui_manager.house_menu.visible = true
	...
	if simulation.ui_manager.house_spawn_button != null:
		...
		if is_tent:
			...
			simulation.ui_manager.house_spawn_button.disabled = slots <= 0 or ordered_today or pending_demolition
			simulation.ui_manager.house_spawn_button.text = "Already ordered today" if ordered_today else ("No free beds" if slots <= 0 else "Order a resident")
		else:
			simulation.ui_manager.house_spawn_button.disabled = slots <= 0 or unhoused > 0 or pending_demolition
			simulation.ui_manager.house_spawn_button.text = "House the initial residents first" if unhoused > 0 else ("No free beds" if slots <= 0 else "Order a resident")
	var settle_button: Button = simulation.ui_manager.house_menu.get_node_or_null("SettleUnhoused") as Button
