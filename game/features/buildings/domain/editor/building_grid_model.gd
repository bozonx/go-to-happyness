...
static func occupied_cells(cell: Vector3i, block_id: StringName, variant: StringName, rot: int) -> Array:
	var fp := BuildingBlockCatalog.footprint_of(block_id, variant)
	if fp == Vector3i.ONE:
		return [cell]
	var ox := (fp.x - 1) / 2
	var oz := (fp.z - 1) / 2
...
func can_place(
	...
	block_id: StringName,
	rot: int = 0,
	material_id: StringName = BuildingMaterialCatalog.DEFAULT_ID,
	variant: StringName = &"",
	anchor: int = 0,
	rot_x: int = 0,
	rot_z: int = 0
) -> bool:
	if not BuildingBlockCatalog.has_block(block_id) or not BuildingMaterialCatalog.has_material(material_id):
		return false
	var norm_variant := BuildingBlockCatalog.normalize_variant(block_id, variant)
	var norm_rot := _normalize_rot(block_id, rot)
	var norm_anchor := BuildingBlockCatalog.normalize_anchor(block_id, norm_variant, anchor)
	var norm_rot_x := ((rot_x % 4) + 4) % 4
	var norm_rot_z := ((rot_z % 4) + 4) % 4
	if not BuildingBlockCatalog.is_valid_anchor(block_id, norm_variant, norm_anchor):
		return false
	var key := placement_key(cell, block_id, norm_variant, norm_anchor, norm_rot, norm_rot_x, norm_rot_z)
	if _blocks.has(key):
		return false
	var candidate := BuildingBlockCatalog.occupied_aabb(cell, block_id, norm_variant, norm_rot, norm_anchor, norm_rot_x, norm_rot_z)
	for existing_key in _candidate_keys(cell, block_id, norm_variant, norm_rot):
		...
		if existing == null:
			continue
		var occupied := BuildingBlockCatalog.occupied_aabb(existing.pos, existing.block_id,
			existing.variant, existing.rot, existing.anchor, existing.rot_x, existing.rot_z)
		if _interiors_intersect(candidate, occupied):
			if not _allows_joint(block_id, existing.block_id) or _same_volume(candidate, occupied):
				return false
	return true
...
func place(
	...
	var norm_variant := BuildingBlockCatalog.normalize_variant(block_id, variant)
	var norm_rot := _normalize_rot(block_id, rot)
	var block := BlueprintBlock.new(cell, block_id, norm_rot, material_id, norm_variant,
		BuildingBlockCatalog.normalize_anchor(block_id, norm_variant, anchor),
...
func write_to_blueprint(blueprint: BuildingBlueprint) -> void:
	blueprint.clear_blocks()
	var keys: Array = _blocks.keys()
	keys.sort()
	for key in keys:
		var block: BlueprintBlock = _blocks[key]
		blueprint.blocks.append(BlueprintBlock.new(block.pos, block.block_id, block.rot,
			block.material_id, block.variant, block.anchor, block.rot_x, block.rot_z))


func load_from_blueprint(blueprint: BuildingBlueprint) -> void:
	clear()
	for block in blueprint.blocks:
		if BuildingBlockCatalog.has_block(block.block_id) and BuildingMaterialCatalog.has_material(block.material_id):
			place(block.pos, block.block_id, block.rot, block.material_id,
				block.variant, block.anchor, block.rot_x, block.rot_z)
...
static func _allows_joint(new_block_id: StringName, existing_block_id: StringName) -> bool:
	return BuildingBlockCatalog.are_compatible_columns(new_block_id, existing_block_id)
