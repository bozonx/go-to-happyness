...
func on_resource_delivered(worker: Citizen, resource_type: String, amount: int) -> void:
	...
	var warehouse_index: int = reserved_index if reserved_index >= 0 else simulation.storage_routing_service.find_reachable_warehouse_index(_drop_position(worker), resource_type, amount)
...
func _finish_storage_delivery(worker: Citizen, resource_type: String, _storage_status := SettlementState.StorageAvailability.OK) -> void:
	if simulation.warehouse_positions.is_empty():
		...
		return
	var next_index: int = simulation.storage_routing_service.find_reachable_warehouse_index(_drop_position(worker), resource_type, 1)
	if next_index >= 0:
		worker.storage_delivery_result(true)
		return
	worker.idle()
