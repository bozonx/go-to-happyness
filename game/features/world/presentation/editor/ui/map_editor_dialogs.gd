...
signal generate_requested(id: StringName, name: String, board_cells: int, recipe_path: String, seed: int)
...
func _on_new_confirmed() -> void:
	...
	if display_name.is_empty():
		display_name = id
	var board_cells := int(_new_board_option.get_item_metadata(_new_board_option.selected))
	var recipe_path := _selected_recipe_path()
	if _new_generate_check.button_pressed and not recipe_path.is_empty():
		generate_requested.emit(
			StringName(id), display_name, board_cells, recipe_path, int(_new_seed_spin.value))
		return
