...
*   туда пойдет юнит из резерва, игрок не может пойти, так как он не может пропасть. Жителю выдается приказ `&"trade_trip"`.
...
*   Через 2 часа юнит спавнится у камня и самостоятельно относит товары на склад (если склада нет то он просто сбрасывает на пол и образуется куча которую нужно разобрать  - это дефолтное поведение при отсутствии склада).
...
*   При падении Wellbeing до 0% житель впадает в депрессию и при первой возможности уходит из поселения во внешний мир через въездной камень.

когда уйдит последний житель то приходит 1 новый и приносит немного денег, еды и комплект перчато чтобы не было софт лока, тоесть игра не допускает того чтобы на карте остался только один игрок, обазятельно должен быть еще хотябы 1 npc
