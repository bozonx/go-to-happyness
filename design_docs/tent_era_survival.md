...
Камень, символизирующий въезд на поляну, является единственным связующим звеном с внешним миром на данном этапе.

### 3.1. Механика заказа
1.  Игрок кликает на въездной камень для открытия интерфейса заказа.
2.  Начальный ассортимент ограничен: **Еда (сухпайки)** и **Строительные перчатки**.
...
*   Игрок должен назначить любого свободного жителя (или пойти сам) в экспедицию к камню. Жителю выдается приказ `&"trade_trip"`.
*   Юнит подходит к въездному камню и исчезает с карты (уходит во внешний мир).
*   **Время отсутствия:** 2 часа внутриигрового времени (юнит временно исчезает с карты).
*   Через 2 часа юнит спавнится у камня с рюкзаком, содержащим купленные товары, и самостоятельно относит их на склад (или складывает в кучу у костра, если склада еще нет).
...
graph TD
    ...
    NoWarehouse -- Нет --> StatusNoWarehouse[Статус: 'No Warehouse' при попытке сбора]
...
*   **Блокировка сбора:** Пока не построен склад, поселенцы не могут накапливать ресурсы. При попытке отправить юнита собирать ветки или траву, он выполняет анимацию сбора, но не может положить ресурс в инвентарь, сбрасывает его и получает статус **«Нет склада»** (`status_no_warehouse`).
*   **Постройка склада:**
...
*   **Разблокировка Костра 1-го уровня (Campfire Lvl 1):**
    *   Как только склад построен, поселенцы могут собирать ресурсы впрок. Игрок может разжечь (улучшить стартовое место горения до полноценного Костра 1-го уровня).
    ...
    *   **Эффекты Костра 1-го уровня:**
        ...
        *   Изначально разблокировано строительство только *Временной палатки* (Temporary Tent). Все остальные здания (костер для готовки, палатки 1 ур. и т.д.) заблокированы и должны быть изучены.
...
*   При падении Wellbeing до 0% житель впадает в депрессию и при первой возможности уходит из поселения во внешний мир через въездной камень (в текущей версии механика ухода не реализована, но проектируется под этот дебаф).
...
### 7.1. Внедорожник энтузиастов (Интерактивная стартовая точка)
На въезде, рядом с камнем, стоит старый сломанный внедорожник, на котором приехали герои. Машина — это ценный, но невосполнимый ресурс:
*   **Выбор игрока:**
    *   *Разбор на запчасти:* Позволяет разобрать корпус и внутренности машины. Дает металл, провода, аккумулятор, фары. Это резко ускорит исследования или позволит построить защищенный металлический ящик для еды (защита от барсуков). Машина исчезает.
    *   *Использование аккумулятора:* Можно использовать генератор/аккумулятор машины для зарядки телефонов (ускоряет исследования) или для питания прожектора, освещающего стройку ночью (позволяет работать ночью без штрафа за темноту). Но топливо/заряд ограничены.
    *   *Элитное спальное место:* В машине может спать 1 человек со 100% комфортом (без дебафов сна на земле). Это может вызвать зависть у остальных, если жилья на всех не хватает.

### 7.2. "Посиделки у костра" (Campfire Stories)
...
### 7.3. Сушка дров и сырого сырья (Интерактивное противодействие погоде)
Погода не должна быть просто раздражающим штрафом, она должна создавать логистические задачи:
*   **Сырые дрова:** После дождя все ветки на земле становятся сырыми.
    *   Если бросить сырые дрова в костер, он начинает сильно дымить (дебаф «Слезящиеся глаза»: скорость работы в радиусе костра падает на 30%) и прогорает в 2 раза быстрее.
    *   Игрок может построить простое здание — **Навес для сушки дров** (Wood Drying Rack). Сложенные туда сырые дрова за 12 часов превращаются в сухие.
*   Это мотивирует игрока заранее планировать запасы сухих дров под навесом до наступления прогнозируемого дождя.

### 7.4. Дикое собирательство: Риск отравления
Сбор грибов и ягод в лесу на старте не является безопасным:
*   Без развитого навыка собирательства жители могут принести на склад ядовитые грибы или недозрелые ягоды (они отображаются как "Неопознанные лесные дары").
*   **Выбор игрока:**
    *   Рискнуть и скормить их жителям (сэкономив нормальную еду). Риск: пищевое отравление (юнит заболевает, постоянно бегает в кусты, теряет энергию и Wellbeing).
    *   Потратить время у костра для готовки, чтобы отварить их (делает еду безопасной, но тратит дрова и время).
    *   Выбросить сомнительные припасы.

### 7.5. Случайные гости и бартер
