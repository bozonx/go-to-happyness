...
*   **Постройка склада:**
    ...
    4.  Ранние поручения являются дневными приказами. Если житель сегодня помогает стройке, собирает траву или работает «Помощником», поручение сбрасывается в конце рабочего дня. Вечером игрок может назначить новые поручения на следующий `workday_id`; утром жители начнут выполнять именно их.
