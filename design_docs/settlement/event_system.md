...
3. **Флаги вместо переменных.** Прежние `protected_firewood_day` и `smoky_firewood_day`
   заменены флагами `firewood_protected_today` и `smoky_firewood` в журнале; проверки
   дневных правил и множителя работы от дыма читают журнал.
