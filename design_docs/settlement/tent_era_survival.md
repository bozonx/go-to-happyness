...
> **Статус:** сценарий реализован; риск-дизайн (§5.2.1) — целевая модель,
> штраф репутации и растущий интервал прихода выживших ещё не написаны.
...
- [world_environment.md](../engine/world_environment.md) — как устроены прогноз, облачность, ветер и небо.
...
*   **Заказ новых жителей через временную палатку не идёт.** Палатка живёт одну ночь, а новый житель приезжает надолго: приглашать людей в жильё, которого утром не будет, — обещание, которое игра не сдержит. Заказ жителя доступен из постоянного жилья.
...
Как приходит фронт и как это выглядит на небе — в
[world_environment.md](../engine/world_environment.md); здесь только последствия для поселения.

**Потепление, похолодание и дождь — словарь поселения, а не движка.** Окружение мира
знает только погодные паттерны; поселение объявляет игроку свои три прогноза и
сопоставляет их с паттернами снаружи ([world_environment.md](../engine/world_environment.md)
§7). Поэтому «холодно» перестаёт быть флагом прогноза и становится
`felt_temperature` из снимка: при **+5 °C и ниже** включаются удвоенный штраф
ночлега и расход калорий ×1,25. Название дня остаётся только текстом для игрока.
