...
| **Вычисляется** | Класс уклона грани (0–6) | `int` | из перепада соседей |
...
| **1** | `gentle` | 11.25° | $1 : 4$ | **4** | Пологий холм, серпантины, съезды для телег | Плавный переход дёрна |
| **2** | `moderate` | 22.5° | $1 : 2$ | **2** | Естественные холмы, обычные съезды | Сглаженный меш с травой |
| **3** | `steep` | 45° | $1 : 1$ | 1 | Овраги, насыпи | Крутая насыпь / осыпь |
| **4** | `very_steep` | 63.4° | $2 : 1$ | 1 | Скалистая осыпь | Текстура осыпи |
| **5** | `pre_cliff` | 76.0° | $4 : 1$ | 1 | Укреплённый эскарп | Скалы / подпорные распорки |
| **6** | `cliff` | 90° | $n : 0$ | 0 | Отвесный обрыв любой высоты | **Авто-скала** или подпорная стенка |

### 3.1. Как это работает на многоклеточных пандусах

Класс 1 (`gentle`) — это **один объект, растянутый на 4 ячейки**, а не 4 независимые
...
шагом $\Delta h / 4 = 0.125$ м; данные при этом остаются целочисленными.
...
Инструмент «Пандус» ставит такой объект целиком: игрок выбирает класс и направление,
система проверяет, что нужные 4 (или 2) клетки свободны и что перепад между конечными
колонками равен `rise`. Если не сходится — пандус не ставится, подсвечивается красным.
Частичный пандус в данных невозможен.

### 3.2. Класс `cliff` — не пандус
...
| Трава / дёрн | 45° | 3 |
| Камень / скала | 76.0° | 5 |
| Подпорная стена (конструкция) | 90° | 6 |
...
* **Triplanar** включается только на гранях класса 4–6, где UV по верхней плоскости
  растягивались бы. На плоскости и пологих склонах — обычные UV, это дешевле.
* **Авто-скала.** Боковые грани класса 6 получают отдельный surface с каменным
...
- `TravelerProfile` получает поле `max_slope_class`: пешеход — 4, тележка — 2, тяжёлый
  груз — 1.
...
| :--- | :--- | :--- | :--- |
| 0–2 | 0° – 22.5° | 100% | 100% (класс 2 — 70%) |
| 3 | 45° | 50% | непроходимо |
| 4 | 63.4° | 25% | непроходимо |
| 5–6 | 76° – 90° | непроходимо | непроходимо |

Классы 5–6 требуют лестниц, серпантинов или подъёмников.
...
- пандус класса 1 ставится только при перепаде ровно 1 шаг на 4 свободные клетки.
