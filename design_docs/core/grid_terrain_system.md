...
Конкретно, для ячейки **со склоном** (плоская колонка в этом проходе не участвует —
иначе терраса перестала бы быть отвесной):
...
2. Пройти 4 ортогональных соседа и по каждой общей паре углов поднять свой угол до
   соседского, но **не более чем на один `rise` своего класса**. Соседские углы берутся
   из их собственных дескрипторов, без рекурсии.
3. Дополнительно: односкатная ячейка (`run = 1`) поднимает грань к плоскому соседу,
   который стоит ровно на один `rise` выше.

Пункт 2 — это и есть §3.4 на практике. Без него склон холма распадается на пандусы вдоль
осей, а по каждой диагонали остаётся вертикальный клин: два перпендикулярных пандуса
поднимают разные углы одной точки. С ним общий угол просто поднимается к обоим, и
внутренний угол холма получается сам. Ограничение «не более одного `rise`» не даёт
подтянуть ячейку к настоящему обрыву на соседнюю террасу.
