...
[Фаза 4a: Материалы и шейдеры] — terrain_materials.md §11
   ├── Каталог 13 материалов, cliff_material, разовая перенумерация индексов
   ├── Детальный байт variant | wear | snow в сетке и в сейве
   ├── Texture2DArray + индекс-карта + два шейдера, height-based блендинг
   ├── Cutout Hole Mask без discard (§6)
   └── Износ поверхности и объединение со счётчиком троп

[Фаза 4b: Вода]
   ├── WaterGrid: уровень, body_id, флаги (§9.3)
   ├── WaterBody и реестр: тип, волна, солёность, течение, рыбный запас (§9.2)
   ├── Брод, снос течением, береговая пена, волны от ветра (§9.5, §9.7)
   └── Лёд как состояние воды, несущая способность в TravelerProfile (§9.6)
