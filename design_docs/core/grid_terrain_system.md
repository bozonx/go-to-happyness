...
variant: int           # 0..15, визуальный вариант текстуры (§7.3, не влияет на геймплей)
wear: int              # 0..2, износ поверхности от хождения (§7.4)
...
| :--- | :--- | :--- |
| Песок, ил, снег, марсианский реголит | 22.5° | 3 |
| Земля, грязь, трава, дёрн, лунный реголит | 45° | 4 |
| Камень, скала, застывшая лава | 76.0° | 6 |
| Лёд, подпорная стена (конструкция) | 90° | 7 |
...
Материал — не только визуал: он задаёт угол осыпания (§4.2), вес прохода в `NavGrid`,
выход грунта при копке (§8) и, для некоторых материалов, дополнительные свойства
(скольжение, износ, требование снаряжения).

#### Трава

| ID | Назначение | Repose | Nav вес | Грунт | Особенности |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `grass` | Основа поселения, луга | 45° (4) | 1.0 | soil | — |
| `grass_tall` | Высокие луга, поля | 45° (4) | 2.0 → 1.0 | soil | Усложняет проходимость, пока не потоптана (§7.4) |
| `grass_swamp` | Болота, топи | 45° (4) | 2.0 | soil+mud | Камыши как декор-оверлей, замедляет |
| `grass_scorched` | Выжженые локации | 45° (4) | 1.2 | ash | После пожара / засухи |

#### Земля

| ID | Назначение | Repose | Nav вес | Грунт | Особенности |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `dirt` | Дороги, тропинки, огороды | 45° (4) | 1.0 | soil | — |
| `dirt_dry` | Засушливые регионы | 45° (4) | 1.0 | soil | Потрескавшаяся текстура |
| `dirt_scorched` | Выжженая земля | 45° (4) | 1.0 | ash | После пожара |
| `lava_cooled` | Застывшая лава | 76° (6) | 1.5 | stone | Каменистая, неровная |

#### Камень и песок

| ID | Назначение | Repose | Nav вес | Грунт | Особенности |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `stone` | Горы, обрывы, скалы | 76° (6) | 1.2 | stone | — |
| `sand` | Пляжи, берега, дюны | 22.5° (3) | 1.3 | sand | Осыпается |

#### Снег и лёд

| ID | Назначение | Repose | Nav вес | Грунт | Особенности |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `snow` | Высокогорье, зима | 22.5° (3) | 1.5 | snow | Глубокий снег, замедляет |
| `ice` | Замёрзшая вода, ледники | 90° (7) | 0.7 | ice | Скользкий: ускорение, repose отвесный |

#### Дно водоёмов

Материал дна определяет тип водоёма поверх него (§9): волну, цвет, течение, рыбу.

| ID | Под какой водой | Repose | Nav вес | Грунт | Особенности |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `seabed` | Море | 22.5° (3) | 2.0 | mud | Ил, ракушки, большие волны |
| `riverbed` | Река | 22.5° (3) | 1.5 | gravel | Галька, течение |
| `lakebed` | Озеро | 22.5° (3) | 1.5 | mud | Тина, ил, тихая вода |

#### Экзоповерхности

| ID | Назначение | Repose | Nav вес | Грунт | Особенности |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `lunar` | Лунная поверхность | 45° (4) | 1.0 | regolith | **Требует скафандр** (§7.5) |
| `mars` | Марсианская поверхность | 22.5° (3) | 1.1 | regolith | **Требует скафандр** (§7.5), осыпается |

**Итого: 17 материалов.** Индекс материала хранится в 1 байте (до 256 значений с
запасом). Визуальные варианты текстур (§7.3) не увеличивают каталог материалов.

### 7.2. Алгоритм смешивания (Index + Height-Based Blending)

С 17 материалами подход «4 канала RGBA8 = 4 веса» (прежняя версия) больше не
масштабируется. Новый подход — **индекс материала + blend-веса**.

* **Индекс материала на ячейку** — `uint8`, хранится в `TerrainGrid` (§2.3). Это
  основной материал ячейки, он определяет все геймплейные свойства.
* **Blend-карта** — `ImageTexture` формата `RG8` с разрешением 1 тексель на ячейку
  ($96 \times 96$ пикселей). Два канала — веса для блендинга с двумя соседними
  материалами на границах. Идентификаторы смешиваемых материалов шейдер берёт из
  индексов соседних ячеек через nearest-сэмплинг.
* **Height-Based Blend**: шейдер учитывает карту высот самой текстуры, поэтому
...
* **Авто-скала.** Боковые грани класса 7 получают отдельный surface с материалом,
  определяемым свойством `cliff_material` материала верха колонки: камень → каменная
  осыпь, песок → песчаная осыпь, лёд → ледяная грань, и т.д.

### 7.3. Визуальные варианты текстур

Каждый материал имеет 1–8 визуальных вариантов (`variant: int` в ячейке, §2.3). Вариант
**не влияет на геймплей**: те же repose, nav вес, тип грунта. Он выбирается кистью
«детали» или детерминированно по координате ячейки (чтобы не хранить для
процедурно-сгенерированных карт).

| Материал | Варианты |
| :--- | :--- |
| `grass` | низкая, средняя, высокая, с цветами 1, с цветами 2, с цветами 3, с цветами 4, с цветами 5 |
| `grass_tall` | высокая, с цветами, с папоротником |
| `grass_swamp` | с камышами, с мхом |
| `grass_scorched` | тлеющая, остывшая |
| `dirt` | обычная, тёмная, с галькой |
| `dirt_dry` | потрескавшаяся, пыльная |
| `stone` | серый, тёмный, песчаник |
| `sand` | жёлтый, белый, крупнозернистый |
| `snow` | пушистый, утоптанный, наст (корка) |
| `ice` | гладкий, с трещинами, торосы |
| `lunar` | светло-серый, тёмный |
| `mars` | красный, оранжевый |

Цветы (тип 1–5) — не отдельный материал, а **вариант травы**: кисть «цветы» меняет
`variant` ячейки на вариант с цветами нужного типа. Для точечной посадки отдельных
цветов используется декор-оверлей через `MultiMeshInstance3D` (§16), но для массового
покрытия поля цветами достаточно варианта текстуры.

### 7.4. Износ поверхности (Wear)

`wear: int` (0–2) на ячейку — состояние поверхности под ногами. Изменяется от
прохождения граждан и восстанавливается со временем.

| Wear | Состояние | Nav вес `grass_tall` | Визуал |
| :--- | :--- | :--- | :--- |
| 0 | Нетронутая | 2.0 | Полная высота травы / декор |
| 1 | Примятая | 1.5 | Трава прижата, декор частично убран |
| 2 | Протоптанная тропа | 1.0 | Голая земля или короткая трава |

**Механика:**
- Гражданин переходит через ребро ячейки → `wear` увеличивается на 1 (не каждый кадр,
  а по факту перехода). Прирост кэшируется и применяется не чаще одного раза в
  симуляционный тик на ячейку, чтобы толпа не изнашивала землю мгновенно.
- Для `grass_tall` nav вес падает с 2.0 до 1.0 по мере протаптывания. Для `grass` и
  `grass_swamp` износ влияет только на визуал, nav вес не меняется.
- Восстановление: если ячейка не посещалась N симуляционных дней → `wear` уменьшается
  на 1. N — параметр материала (трава восстанавливается быстрее, камень не
  восстанавливается вообще).
- `wear` хранится в ячейке (§2.3) и сохраняется (§12).

Это та же система, что и протаптывание троп от хождения: единый механизм износа
поверхности, работающий для любого материала с `wear` > 0.

### 7.5. Требование скафандра (экзоповерхности)

Материалы `lunar` и `mars` — поверхности без атмосферы. Гражданин без скафандра не
может на них зайти.

**В NavGrid:**
- `TravelerProfile.has_suit: bool` — флаг наличия скафандра.
- Ребро к ячейке с `lunar`/`mars` непроходимо, если `has_suit == false`.
- Проверка выполняется в `is_step_passable` наряду с `max_slope_class` и глубиной воды.

**В геймплее:**
- В survival скафандр — предмет экипировки, производимый через цепочку исследований.
  До его получения экзоповерхности — естественный барьер, ограничивающий экспансию.
- В песочнице — опция: по умолчанию `has_suit = true` для всех граждан.
- Гражданин в скафандре движется с nav весом материала (1.0 для `lunar`, 1.1 для
  `mars`), без штрафа за скафандр. Скафандр ограничивает доступ, а не скорость.
...
2. **Тип водоёма определяется материалом дна (§7.1), а не типом воды.** Вода — единый
   слой; шейдер читает материал дна под собой и меняет поведение:

   | Материал дна | Волны | Цвет | Течение | Рыба | Урон |
   | :--- | :--- | :--- | :--- | :--- | :--- |
   | `seabed` | Большие | Сине-зелёный | Приливные (опц.) | Морская | нет |
   | `riverbed` | Средние | Зелёно-синий | **Направление + скорость** | Пресная | нет |
   | `lakebed` | Минимальные | Тёмно-синий | нет | Пресная | нет |
   | `lava_cooled` | Бурлящие | Оранжевый (Emission) | нет | нет | **да** |

3. **Течение рек.** Для ячеек с `riverbed` под водой хранится вектор течения
   `(dir: int 0..7, strength: int 0..3)` в слое воды. Направление задаётся кистью
   реки. Течение влияет на визуал (анимация потока) и на снос граждан/предметов в
   потоке. Гражданин, входящий в реку с течением, получает боковой снос,
   пропорциональный `strength`.
4. **Прибрежная пена:** на стыке воды и склонов шейдер рендерит прибой. Интенсивность
   пены зависит от материала дна: `seabed` — активный прибой, `lakebed` — едва
   заметный.
5. **Лёд зимой.** При сезонном похолодании вода над `lakebed`/`riverbed` замерзает:
   поверхность заменяется на материал `ice` (§7.1). Морская вода (`seabed`) замерзает
   только в полярных биомах. Лава не замерзает.
6. **Навигация.** Глубина = уровень воды минус высота колонки.
   ...
   * Течение в брод: снос пропорционален `strength`, при `strength` $\ge 3$ брод
     непроходим.
...
- `TravelerProfile.max_slope_class`: пешеход — 5, тележка — 3, тяжёлый груз — 2.
  ...
  не меняет.
- `TravelerProfile.has_suit`: если `false`, рёбра к ячейкам с `lunar`/`mars` (§7.5)
  непроходимы. В survival по умолчанию `false`, в песочнице — `true`.
...
- Формат чанка: `height` — `PackedByteArray` со смещением (1 байт); `material_index` —
  байтовый индекс в каталог (1 байт); `variant` (4 бита) + `wear` (2 бита) + 2 spare
  бита — упакованы в 1 байт; `slope_id`/`slope_dir`/`slope_index`/`flags` — упакованы
  в один байт. Итого 4 байта на ячейку, $\approx 37$ КБ на карту $96 \times 96$ —
  сжатие не требуется.
...
game/features/world/
  domain/terrain/
    ...
    terrain_material_catalog.gd # §4.2, §7.1: материал → угол осыпания, nav вес, грунт
...
Материалы и износ:
- каталог материалов §7.1 содержит 17 записей, каждой соответствует корректный угол
  осыпания из §4.2;
- `variant` не влияет на repose, nav вес и тип грунта — только на визуал;
- `wear` увеличивается от перехода через ребро и уменьшается после N дней без
  посещения;
- `grass_tall` с `wear=0` даёт nav вес 2.0, с `wear=2` — 1.0;
- рёбра к `lunar`/`mars` непроходимы при `has_suit=false` и проходимы при
  `has_suit=true`;
- материал дна (`seabed`/`riverbed`/`lakebed`) определяет тип воды поверх него.
...
[Фаза 4: Вырезы, вода и шейдеры]
   ├── Расширенный каталог материалов (17 типов) + варианты текстур (§7.1, §7.3)
   ├── Index + Height-Based Blending, авто-скала (§7.2)
   ├── Cutout Hole Mask без discard (§6)
   ├── Дискретный слой воды, брод, береговая пена (§9)
   ├── Износ поверхности (wear) и протаптывание (§7.4)
   └── Скафандр для экзоповерхностей (§7.5)
