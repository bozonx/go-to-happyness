...
> **Статус:** реализовано до фазы 4b включительно (§15); 4c (сезоны) и 5 (чертежи и
> земляные работы) — спецификация
> **Целевая аудитория:** Подростки и взрослые (14+), ценящие креативность, точность и архитектурный контроль.
> **Дочерние документы:**
> [terrain_materials.md](terrain_materials.md) — каталог материалов, варианты, износ, снег,
> сезоны, блендинг и текстурный бюджет;
> [water.md](water.md) — водоёмы, лёд, течение и навигация в воде.
> Здесь — форма земли: высоты, уклоны, каскад, вырезы и мешер.
...
| **Вычисляется** | Переход грани | класс 0–7 + знаковый перепад, м | из углов и высоты соседей |
...
* **Горизонтальная ячейка:** $1.0 \times 1.0$ м (совпадает с
  `BuildingBlueprints.BLOCK_SIZE`, то есть с клеткой строительной сетки).
...
* **Размер доски:** свойство карты, а не движка: выбирается пресетом при её
  создании ([map_editor.md](map_editor.md) §6.2). Сетка работает на любом размере,
  кратном `CHUNK_CELLS`.
...
| **1** | `shallow` | 3.58° | $1 : 8$ | **8** | Длинные пологие дороги и серпантины для тяжёлого груза | Едва заметный уклон дёрна |
| **2** | `gentle` | 7.13° | $1 : 4$ | **4** | Пологий холм, серпантины, съезды для телег | Плавный переход дёрна |
| **3** | `moderate` | 14.04° | $1 : 2$ | **2** | Естественные холмы, обычные съезды | Сглаженный меш с травой |
| **4** | `steep` | 26.57° | $1 : 1$ | 1 | Овраги, насыпи | Крутая насыпь / осыпь |
| **5** | `very_steep` | 45° | $2 : 1$ | 1 | Скалистая осыпь | Текстура осыпи |
| **6** | `pre_cliff` | 63.43° | $4 : 1$ | 1 | Укреплённый эскарп | Скалы / подпорные распорки |
...
а не меняет принцип таблицы. Угол — физический `atan(rise × Δh / (run × cell_size))`;
UI, навигация и `CharacterBody3D.floor_max_angle` выводят его из тех же величин.
...
```
Профиль gentle (7.13°), подъём 1 шаг на 4 клетки:
...
В Редакторе Карт инструмент называется «Соединить высоты»: автор протягивает его между
двумя площадками, направление определяется их положением, а класс по умолчанию — парой
`rise : run` между точками. Неровные промежуточные колонки выравниваются к основанию и
получают дескрипторы пандуса **одной транзакцией**; конечные площадки инструмент не
двигает. Перепад больше `rise` выбранного класса собирается цепочкой одинаковых звеньев.
Точный класс можно закрепить в расширенной настройке. Пандус под курсором можно сделать
«Пологее» или «Круче»: редактор находит истинные низ и верх всей цепочки, расширяет или
сокращает её след, распускает лишние старые звенья и коммитит результат одной дельтой.
Вырез, якорь, поперечный чужой пандус, диагональ или пара, которой нет в каталоге,
отклоняют весь жест и подсвечиваются красным. Частичный пандус в данных невозможен. Лаборатория сохраняет низкоуровневую
установку по классу и направлению для отладки мешера. В обычном игровом инструменте
высоты (§3.2) игрок классом не управляет напрямую, он вычисляется автоматически.

### 3.2. Авто-выбор класса на границе (Auto-Slope Selection)

Игровой инструмент высоты не даёт игроку выбирать `slope_id`. В Редакторе Карт автор
может оставить ту же политику «естественная» или закрепить профиль для Sculpt/Level.
После
...
2. Посчитать **бюджет клеток** для этой границы: $\lceil \text{перепад} / \text{repose} \rceil$,
   ...
   дополнительно ограничен константой `MAX_CHAIN_CELLS` (64).
...
оформляет свою границу одной клеткой класса 4 (26.57°), песок — террасой в две клетки
класса 3 (14.04°), а скала
...
**Авторский профиль Sculpt/Level.** Если Редактор Карт закрепил класс, его
`rise / run` временно становится углом осыпания самой операции вместо свойства
материала. Поэтому каскад заранее досыпает или срезает нужное число целочисленных
колонок, а `SlopeAssigner` рассматривает только выбранный класс. Например, подъём `+3`
с `gentle 1:4` формирует три звена и расходует 12 клеток. Это не косметическая замена
дескриптора после каскада: высоты и пандусы вычисляются и откатываются вместе.
...
Высоты 4 углов ячейки вычисляются детерминированно из её собственного дескриптора склона и высот 4 ортогональных соседей;
...
* **Не выше чем на 2 шага над своей колонкой.** Ровно столько «стоит» диагональ склона
  класса 4: по шагу в каждую сторону. Именно поэтому угол между четырьмя такими клетками
...
| :--- | :--- | :--- |
| `sand`, `mud`, `mars_regolith` | 14.04° | 3 |
| `grass`, `grass_tall`, `dirt`, `scorched`, `gravel`, `lunar_regolith` | 26.57° | 4 |
| `stone`, `lunar_rock`, `mars_rock` | 63.43° | 6 |
| `ice` (ледник), подпорная стена (конструкция) | 90° | 7 |

**Угол осыпания направленный: грань держит верхняя колонка.** Осыпание — это про землю,
которая съезжает **вниз**, поэтому ограничение накладывает перепад, над которым колонка
стоит, а не тот, под которым она лежит. Луг у подножия скалы не держит скалу и остаётся
лугом; песок на бровке двухшагового уступа — авторский обвал и запрещён. Проверка по
модулю разности запрещала оба случая одинаково: кисть материала отказывалась красить
траву рядом с любой горой, а генератор, живущий по тому же правилу, красил камень с обеих
сторон каждой грани и выдавал карты, где камнем было две трети суши
...
- **Якоря.** Ячейки под построенным зданием, дорогой или размещённым чертежом помечены
  ...
  под домом нельзя.
  Флаг ставится транзакцией `TerrainService.set_anchor` — как и всё остальное, через
  единственного владельца записи: якорь, поставленный мимо него, undo молча снимет.
  Для размещённых зданий флаг пишет `BuildingPlacementService` в той же транзакционной
  границе, что merge площадки; освобождение записи снимает только больше никем не
  занятые клетки. Другие будущие виды якорей обязаны иметь столь же явного владельца.
...
1. **Выбор якорной высоты.** По умолчанию — медиана высот колонок под пятном застройки;
   остальные режимы (по верху, по низу, вручную) — `building_placement.md` §4.2.
...
3. **Валидация.** Отклоняется, если: волна каскада задела чужой якорь; перепад по
   границе превышает 4 шага; пятно выходит за доску, пересекает вырез (`is_hole`) или
   чужое пятно; в survival — не хватает грунта (§8). **Вода отказом не является**:
   здание опирается на рельеф, и площадка ниже уровня воды — это пирс, затопленные руины
   или замок в лаве (`building_placement.md` §3).
4. **Превью.** До подтверждения показывается призрак итогового рельефа: срез — синим,
   насыпь — жёлтым, конфликт — красным.
5. **Коммит.** Применяется одной транзакцией с записью дельты в undo; пересчёт водных тел
   входит в ту же транзакцию.

Система вправе **отказать** в размещении. Это не ошибка, а нормальный ответ.

Всё остальное — режимы выравнивания, поведение границы (пандус или терраса), ориентация,
проверка входов, формат записи и политика стройки игрока — в
[building_placement.md](building_placement.md).
...
2. **Геометрия не генерируется.** В вырезанных ячейках мешер **не создаёт полигонов** —
   ни верха, ни граней. Прятать их шейдером нельзя: `discard` ломает depth-prepass и
   тени и, главное, не убирает коллизию — гражданин упрётся в невидимую землю в проёме.
...
индекс-карту размером с доску (`terrain_materials.md` §7.3), а не через вершинный атрибут, поэтому
...
3. **Приказ «Подготовить площадку».** Игрок выделяет прямоугольник и целевую высоту.
   Создаётся заказ в существующей системе ордеров ([order_system.md](../citizens/order_system.md)); рабочие с лопатами
...
Вода — **второй дискретный слой над той же доской**, с тем же шагом $\Delta h$. Она
авторская, а не симулируемая; глубина не хранится, а выводится как уровень минус высота
колонки. После завершённой правки рельефа водный сервис атомарно перестраивает контур,
удаляет засыпанный водоём или разделяет его поднятой перемычкой.

Модель водоёма, лёд, течение, навигация в воде и формат `water.bin` вынесены в
**[water.md](water.md)** — там это одна связная тема со своим редакторским режимом и
своим планом.

Сетке от воды не нужно ничего: `TerrainGrid` про воду не знает. Обратная зависимость
существует и описана там же: `WaterService` читает высоты, `BorderOceanService`
подписан на `TerrainService.edit_committed`, а `Placement Merge` (§5.3) отклоняет пятно,
пересекающее воду.
...
поверхности, класс уклона и знаковый перепад высоты перехода к каждому из восьми соседей. Классы там — обычные
...
- Ребро считается разрывом (класс 7), если общие углы двух колонок расходятся хотя бы
  на пол-шага; иначе класс берётся по фактическому набору высоты на клетку. Разрыв не
  симметричен: `edge_height_delta > 0` проверяется по `max_step_up`, отрицательный — по
  `max_drop`. Для пешехода это 0.5 м вверх и 1.0 м вниз; вырез и край доски не имеют
  конечного перепада и остаются непроходимы.
...
- `TravelerProfile.max_slope_class`: пешеход — 5, тележка — 3, тяжёлый груз — 2.
  ...
  не меняет.
- `TravelerProfile.has_suit`: **спецификация фазы 4c, не реализовано.** Когда появится,
  бит `airless` будет публиковаться в `NavTerrainField` **из региона карты, а не из
  материала** (`terrain_materials.md` §8): «нет атмосферы» — свойство места, иначе
  марсианский и земной базальт обязаны быть разными материалами. Сегодня поля навигации
  такого бита не несут, и понятия региона в мире ещё нет.
...
Поле навигации несёт, помимо формы земли, два производных бита на ячейку — `frozen` и
`is_water` (третий, `airless`, приедет с регионами в фазе 4c) — и множитель веса от снега
и износа. Всё это **веса и
...
| :--- | :--- | :--- | :--- |
| 0–3 | 0° – 14.04° | 100% | 100% (класс 3 — 70%) |
| 4 | 26.57° | 50% | непроходимо |
| 5 | 45° | 25% | непроходимо |
| 6–7 | 63.43° – 90° | непроходимо | непроходимо |
...
Направленный переход добавляет множитель подъёму; спуск сохраняет базовую цену и потому
дешевле обратного пути, не опуская эвристику ниже её допустимого минимума.
...
(см. [navigation_and_roads.md](navigation_and_roads.md), «Графы пространства»), уровень
перестанет быть всегда нулевым без правки вызывающих.
...
Замеры (`tests/repro/diag_terrain_nav_publish_cost.gd`). Доска $96\times96$ — не
дефолт: пресет по умолчанию `map_editor.md` §6.2 — $256\times256$, и там всё дороже
примерно в 7 раз, пропорционально числу колонок.

| Операция | $96\times96$ | $256\times256$ |
| :--- | :--- | :--- |
| Полная публикация | ~370 мс (9216 колонок) | ~2.6 с (65 536 колонок) |
| Направленная связность | ~650 мс | ~4.6 с |
| Штрих кисти 3×3 | ~2 мс | ~2 мс |
| Штрих кисти 9×9 | ~10 мс | ~10 мс |
| Кисть 33×33 | ~100 мс | ~100 мс |

Кисть до 9×9 укладывается в кадр; крупная — на отпускание. Штрих стоит по числу
задетых колонок и не зависит от размера доски — в этом и смысл инкрементальной
публикации.

**Порог отката на полную публикацию.** Инкрементальный путь платит за словарь
раздутых клеток и его сортировку, полный — нет, поэтому на большом пятне он проигрывает.
Замер: заливка всей доски через `refresh_cells` — 660 мс против 349 мс полной публикации
на $96^2$ и 4.76 с против 2.65 с на $256^2$. Поэтому `refresh_cells` при пятне больше
`FULL_PUBLISH_SHARE` (¼ доски) зовёт `publish_all`.

**Правка реестра публикует только свои клетки.** `registry_changed` несёт список
затронутых клеток. У создания пустого водоёма он пуст — и это самый частый случай, потому
что редактор заводит водоём в момент, когда автор выбирает «+ озеро» в палитре, до того
как появилась хоть одна его клетка. Полная перепубликация там стоила 362 мс на $96^2$ и
**2.5 с на $256^2$ — на каждый клик**; теперь 0. **Рёбра читаются из уже
...
В debug-сборке поселения та же клавиша `M` включает этот же оверлей над игровым
`NavGrid`, чтобы отказ маршрута можно было диагностировать без редактора.
...
| Бюджет перестройки | **по времени: 4 мс геометрии + 2.5 мс растительности за кадр** | стоимость чанка отличается на порядок между полосой ровной земли и полной перестройкой холма, поэтому счётчик чанков — не та единица |
...
| LOD | чанки дальше 64 м — верхняя поверхность и **юбка**, без граней между колоннами | камера изометрическая, внутренних граней не видно; юбка — силуэт края доски и вырезов, её ронять нельзя |
| Дельта-обновление | чанк эмитится **полосами по 4 ряда**, мешер живёт вместе с чанком | правка одной колонки переэмитит одну-две полосы вместо 256 колонок и 1024 рёбер |
| Кодирование сглаживания | считается, только когда включён `full_smoothing` или `edge_rounding` | в игре и редакторе карт оба выключены, а это была самая дорогая часть перестройки |

Грязные чанки собираются в очередь и перестраиваются по бюджету времени. Бюджет
всегда пропускает хотя бы один чанк, иначе одна дорогая перестройка заблокировала
бы очередь навсегда.

**Растительность — отдельная очередь.** Трава не несёт коллизии и ничего не решает,
но стоит примерно столько же, сколько сама земля. Она не входит в
`rebuild_pending_now`: доска появляется, трава дорастает за следующие кадры.
`rebuild_everything_now` дожидается и её — это нужно ровно одному вызывающему,
снимку экрана.
...
**Что было сделано после замеров.** Greedy meshing и кэш углов не были узким местом:
на плоском чанке, дающем один квад, уходило 4.6 мс, то есть цена была пропорциональна
КЛЕТКАМ, а не геометрии. Убраны, по убыванию выигрыша: кодирование сглаживания при
выключенных режимах, `PackedFloat32Array` на каждый из 1024 вызовов `_wall_of`,
`Dictionary` на каждое из ~940 рёбер чанка, форматирование строкового ключа линии,
и цепочки вызовов-аксессоров в `corner_heights_into`. Итог на холмистом чанке:
13.5 → 5.0 мс на полной перестройке и 3.3 мс на дельте.
...
- Формат ячейки: `height` — байт со смещением; `material_index` — байтовый индекс в
  каталог; детальный байт `variant` (4 бита) + `wear` (2) + `snow_depth` (2);
  дескриптор `slope_class` (3 бита) + `slope_dir` (3) + `slope_index` (3) + `flags` —
  **два байта, а не один**. Итого **5 байт на ячейку**, $\approx 46$ КБ на карту
  $96 \times 96$ и 320 КБ на $256 \times 256$ — сжатие не требуется.
  Оценка «4 байта» здесь стояла до реализации и предполагала, что дескриптор влезет в
  байт; он не влезает — это одиннадцать бит, и ужать направление до двух бит значило бы
  сломать единственное обещание кодировки направлений (`map_terrain_codec.gd`).
...
- Слой воды пишется отдельно и описан в [water.md](water.md) §3.
...
## 13. Единственный владелец высоты

Высоту и видимую землю публикует только `GridTerrainWorld` над `TerrainGrid`,
создаваемый в `WorldSetup._build_terrain()`. Второго владельца быть не может: высоту
потребляет `routing` (`NavGrid.is_step_passable`, `segment_cost`), и расхождение двух
источников означает жителя, идущего сквозь обрыв.

Поэтому в проекте нет резервной плоскости под доской: `TerritoryBase` её не содержит,
а мир за границей рисует `WaterWorld` по заголовку карты.
...
Файлы материалов, износа, снега и сезонов перечислены в `terrain_materials.md` §8,
файлы воды — в `water.md` §8. Они относятся к тому же `world`, но живут своей темой и
своим планом.
...
Фазы 0–3 (модель данных, мешер, каскад и кисти, интеграция с навигацией), 4a (материалы
и шейдеры) и 4b (вода) закрыты. Кисти живут в `TerrainBrushController`, общем для
...
Якоря `IS_ANCHOR` (§4.4) размещённых зданий **пишет `BuildingPlacementService`** через
`TerrainService`. Один сервис одновременно владеет merge, записью `MapPlacementRecord`
и сроком жизни якоря, поэтому отмена стройки и снос не могут забыть освобождение.
Снимаются только клетки, больше не занятые другой записью. Инвариант покрыт
`tests/domain/test_building_placement.gd`; отдельного слушателя `BuildingRegistry` для
якорей больше нет, поэтому второй владелец не может незаметно вернуться в runtime.
...
| Блендинг материалов | свой шейдер, индекс-карта `R8` по размеру доски + карта деталей `RGBA8` |
...
- `Terrain3D` — непрерывный heightmap-скульптинг несовместим с дискретными террасами:
  ни отвесных граней, ни чистых углов, ни per-cell материала.
...
- `NavigationServer3D` / `NavigationMesh` — сетка маршрутов своя, детерминированная и
  завязанная на веса дорог и троп; navmesh её не заменит.
