...
перетекает по кадрам; заливка низины считается один раз — **по команде автора**
(инструмент Flood), а не как побочный эффект правки рельефа. Гидродинамика в реальном
времени не нужна ни одному игровому решению и стоила бы больше, чем весь терраин.

Правка рельефа воду при этом всё равно меняет — но только через глубину, без единой
записи в водный слой: поднял дно выше уровня, и клетка суха (§9.3). Автоматически
...
**Единственное исключение — граница карты** (`map_editor.md` §6.1). Если в заголовке
карты `border.kind = ocean`, море стоит за доской всегда, и любая низина, связанная с
краем и лежащая ниже `border.level`, — вода. Это не симуляция, а то же однократное
затопление, просто его условие записано в файле карты, а не в жесте автора:
`BorderOceanService` доливает край после каждой правки рельефа, и залив попадает в ту же
команду undo, что и вызвавший его штрих.
...
числа на `LAKE`, а не запись в enum. Это то же правило, которое `terrain_materials.md`
§1 ставит каталогу материалов, и по той же причине: каждый тип стоит строки в пяти
таблицах навсегда. По нему `POND` из первой редакции упразднён — от озера он отличался
только рябью и цветом.
...
**Рыба здесь не живёт.** В первой редакции `WaterBody` нёс `fish_stock` и
`fish_regen_per_day`; поля были удалены до появления первой авторской карты. Рыбный
запас — ресурсное состояние с квотой, переловом и восстановлением, и §9.2 сам это
говорит: он ложится на `world_resource_state.gd`. Держать его словарём в теле водоёма
значило зафиксировать формат данных в файле карты раньше, чем известна механика,
которая его читает.

**Про течение — честно.** Пункт 4 выше отвергает per-cell течение как «разрешающее
физически невозможные конфигурации», но редактор (`map_editor.md` §5.3) даёт автору
писать направление в клетки штрихом кисти и связность русла не проверяет. Разреженная
карта потока у водоёма — правильное хранилище (течение принадлежит руслу, а не карте),
но валидация русла не сделана и в фазу 4b не входила. Пока автор может нарисовать реку,
текущую в гору; это его ответственность, а не инвариант данных.
...
* **Несущая способность.** `ice_thickness` 0–3 против третьего скаляра в
  ...
  Различимых порогов, таким образом, **два**, а не три: значение 3 сегодня не
  отличается от 2 ни для кого. Четвёртое значение — запас двухбитного поля под третий
  класс путника, а не ступень, которую автор чувствует; инструмент честно предлагает
  выбор из трёх, и это ровно на один больше, чем механика различает.
...
* Замерзание и таяние — **единственное из сезонных изменений, которое меняет
  ...
  инкрементальной републикацией (§10.5). Сезон может применить её ко всему водоёму,
  а редактор даёт отдельные кисти Freeze/Thaw для авторских локальных переправ; у
  обоих путей один владелец записи и один undo-маршрут.
...
`TerritoryBase` не содержит резервной плоскости: высоту и видимую землю публикует только
`GridTerrainWorld` выбранной карты, а мир за границей рисует `WaterWorld` по заголовку.
...
game/features/world/
  ...
  domain/water/
    water_grid.gd          # §9.3, три массива: уровень, body_id, флаги, И реестр
    water_body.gd          # §9.2, тип, волна, солёность, течение
    water_delta.gd         # §9, состояние клеток до/после; undo-запись
    water_body_edit.gd     # §9.2, создание и удаление водоёма как транзакция
    water_flow_edit.gd     # §9.2, правка разреженной карты потока
  application/
    ...
    water_access_service.gd # §9.2, берега, с которых можно набрать воду
    border_ocean_service.gd # §6.1 map_editor.md, залив края по заголовку карты
  presentation/
    terrain/
    ...
    water/
      water_world.gd         # §9.5, план водоёма, коллизия льда, океан за границей

Реестр водоёмов живёт **внутри `WaterGrid`**, а не отдельным `water_body_registry.gd`,
как обещала первая редакция: `set_cell` обязан отклонять ссылку на незарегистрированный
водоём, значит реестр обязан быть там же, где ячейки, и грузиться одной операцией с ними.
```
...
[Фаза 0: Снос Terrain3D] — СДЕЛАНО
   ...
   └── GridTerrain как единственный runtime-слой земли
...
[Фаза 4b: Вода] — СДЕЛАНО, кроме рыболовства и сноса течением
   ...
   ├── WaterService: транзакция, залив низины, уровень, осушение, лёд, undo — СДЕЛАНО
   ├── Брод ×3, непроходимая глубина, лава, публикация в NavTerrainField (§9.7) — СДЕЛАНО
   ├── Лёд как состояние воды, min_ice_thickness, коллизия льда (§9.6) — СДЕЛАНО
   ...
   ├── Океан за границей карты и залив края (map_editor.md §6.1) — СДЕЛАНО
   ├── Вода в сессии: слой `WaterGrid` выбранной карты и берега через
   │   WaterAccessService; декоративные пруды удалены — СДЕЛАНО
   ├── Рыболовство — не начато и полей под него в WaterBody больше нет (§9.2)
