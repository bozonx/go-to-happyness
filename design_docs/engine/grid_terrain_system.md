...
- **Якоря.** Ячейки под построенным зданием, дорогой или размещённым чертежом помечены
  ...
  Пишет `TerrainAnchorService`: он слушает `BuildingRegistry`, считает совместные
  закрепления и вызывает `TerrainService`, как подробно описано в §15.
...
Якоря `IS_ANCHOR` (§4.4) **пишет `TerrainAnchorService`**: он слушает
`BuildingRegistry` — единственное место, где след здания занимается и освобождается, —
и закрепляет колонки под ним через `TerrainService`, то есть транзакцией, которую
видит отмена. Закрепления считаются, а не флагуются: два соседних здания, делящих
колонку, должны оба её отпустить. Обе половины инварианта теперь на месте и покрыты
тестами (`tests/domain/test_terrain_anchors.gd`). Фазе 5 остаётся Placement Merge.
