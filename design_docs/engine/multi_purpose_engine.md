...
**Модель времени и UI-layout полей в формате нет.** Они были объявлены раньше своих
потребителей: `clock` и `ui_layout` записывались, редактировались и не читались никем,
так что автор мог выбрать `turn_based` и молча получить realtime. Поле, которое врёт,
хуже отсутствующего, поэтому оба удалены и вернутся вместе с реализацией — turn-based
